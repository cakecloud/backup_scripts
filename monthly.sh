...
if [ $DAY = "342" ]; then
 ...
 rm -rf ${LAST_YEAR}
 mkdir -p ${LAST_YEAR}
 mv ${BACKUP}/${DOMAIN}* ${LAST_YEAR}/
 rm -rf ${SNAPSHOT_FILE_0}
 rm -rf ${BACKUP}/${DOMAIN}*
 rm -rf ${BACKUP}/${DOMAIN}*
 $SSH -p ${SSHPORT} ${SSHUSER}@${SSHSERVER} "mkdir ${DOMAIN}/last_year"
 $SSH -p ${SSHPORT} ${SSHUSER}@${SSHSERVER} "mv ${DOMAIN}/${BKDIR}/* ${DOMAIN}/last_year/"
...
$SCP -P ${SSHPORT} -C ${BACKUP}/${BFILE} ${SSHUSER}@${SSHSERVER}:${SSHDUMPDIR}/${BFILE}
$SCP -P ${SSHPORT} -C ${BACKUP}/${MFILE} ${SSHUSER}@${SSHSERVER}:${SSHDUMPDIR}/${MFILE}
$SCP -P ${SSHPORT} -C ${SNAPSHOT_FILE} ${SSHUSER}@${SSHSERVER}:${SSHDUMPDIR}/
$SCP -P ${SSHPORT} -C ${SNAPSHOT_FILE_0} ${SSHUSER}@${SSHSERVER}:${SSHDUMPDIR}/
